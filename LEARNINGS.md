...
The salt prevents rainbow table attacks on the 16×16 grid.
...
struct Player {
    ...
    int8 defense;    // 1 byte
    uint8 status;    // 1 byte
    // 5 bytes → fits in 1 slot with commitment
    bytes32 commitment;
}
```

### Bitmap > mapping for small fixed sets
16×16 grid = 256 cells = 1 uint256 bitmap for walls/treasures.
...
*More learnings coming as we build toward mainnet... 🛡️*
