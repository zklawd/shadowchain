...
The Solidity verifiers generated by `bb 0.72.1` triggered a "stack too deep" error
during compilation with default optimizer settings.

**Solution:** Set `optimizer_runs = 1` in `foundry.toml`. This gives the compiler
more freedom to optimize for code size over gas efficiency, which reduces stack usage.
...
**Trade-off:** Slightly higher gas costs for verification, but contracts compile and work correctly.
